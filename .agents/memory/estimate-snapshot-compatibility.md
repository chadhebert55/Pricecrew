...
**How to apply:** Keep request schemas strict for new calculations, but keep serializers tolerant of absent additive fields and legacy input shapes; do not require historical job-input JSON to satisfy the latest request validator. Normalize legacy fields only in response serialization, never before persistence. Status/proposal-only updates must reuse stored pricing unchanged; override updates may recalculate only override-derived totals while preserving the original warning representation. Calculate only preview/new-quote flows from current settings, and verify existing inputs, assembly, pricing, warnings, proposals, totals, and margins remain unchanged across schema and seed changes.