...
**How to apply:** Mark every user edit contractor-owned, keep normal startup seeding insert-only, and use narrowly proven reconciliation for legacy system-owned rows. Supplier-file imports must canonicalize and calendar-validate row dates, then recheck ownership, exact identity, and source-date freshness at apply time; applying an older source date to a current system-owned row requires explicit acknowledgement. Serialize imports per tenant, fail closed on conflicting exact identifiers, and preserve verified fields omitted from partial exports. Test fresh initialization, legacy edited-row preservation, and concurrent imports together.