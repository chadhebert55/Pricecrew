...
export function NewCustomQuote() {
  ...
  useEffect(() => {
    if (settings && !settingsLoaded) {
      setInputs((current) => ({
        ...current,
        laborSellRate: settings.residentialLaborSellRate,
            ...
            <Card className="border-t-4 border-t-secondary">
              ...
              <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input id="custom-customer" required value={customerName} onChange={(event) => setCustomerName(event.target.value)} />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="custom-email">Customer Email</Label>
                  <Input id="custom-email" type="email" value={customerEmail} onChange={(event) => setCustomerEmail(event.target.value)} />
                </div>
