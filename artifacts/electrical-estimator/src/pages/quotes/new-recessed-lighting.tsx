import { type RecessedLightingInputs, usePreviewQuote, useGetSettings } from "@workspace/api-client-react"
...
import { RecessedBuilderFields, initialRecessedInputs } from "@/components/recessed-builder-fields"
import { PricingReview, pricingNeedsReview } from "@/components/remodel-builder"

const initialInputs: RecessedLightingInputs = initialRecessedInputs
...
export function NewRecessedLightingQuote() {
  ...
  const [proposalDescription,setProposalDescription] = useState("Provide labor and listed materials for the agreed recessed lighting scope, including selected fixture installation, wiring, lighting controls, testing and cleanup. Final layout, existing circuit capacity, ceiling conditions and fixture/control compatibility will be verified before work begins.")
  ...
  const {draftRecovery} = useQuoteBuilderDraft({module:"RECESSED_LIGHTING",ready:settingsLoaded && !revision.isRevision,
  ...
  useEffect(()=>{
    if(settings && !settingsLoaded && !revision.isRevision) {
      setInputs(current=>({...current,laborAdjustmentHours:settings.recessedLightingLaborAdjustmentHours ?? 0}))
      setSettingsLoaded(true)
    }
  },[settings,settingsLoaded,revision.isRevision])
  const payload={module:"RECESSED_LIGHTING" as const,jobInputs:inputs,laborOverride:optionalAmount(laborOverride),sellingPriceOverride:optionalAmount(sellingPriceOverride)}
  ...
  return <div className="mx-auto max-w-7xl space-y-6 pb-24">
    <div><h1 className="text-3xl font-bold tracking-tight">New Recessed Lighting Quote</h1><p className="mt-1 text-muted-foreground">Recessed Lighting Electrical Builder</p></div>
      ...
      <div className="min-w-0 space-y-6 xl:col-span-2">
        <Card className="border-t-4 border-t-secondary"><CardHeader><CardTitle>Project Details</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2"><Label htmlFor="recessed-customer">Customer Name *</Label><Input id="recessed-customer" required value={customerName} onChange={e=>{setCustomerId(undefined);setCustomerName(e.target.value)}}/></div>
            <div className="space-y-2"><Label htmlFor="recessed-email">Customer Email</Label><Input id="recessed-email" type="email" value={customerEmail} onChange={e=>{setCustomerId(undefined);setCustomerEmail(e.target.value)}}/></div>
            <div className="space-y-2 md:col-span-2"><Label htmlFor="recessed-project">Project Name *</Label><Input id="recessed-project" required value={projectName} onChange={e=>setProjectName(e.target.value)}/></div>
            <div className="space-y-2 md:col-span-2"><Label htmlFor="recessed-proposal">Customer-facing Proposal Description *</Label><Textarea id="recessed-proposal" required value={proposalDescription} onChange={e=>setProposalDescription(e.target.value)}/></div>
          </CardContent>
        </Card>
        <Card className="border-t-4 border-t-primary"><CardHeader className="bg-primary/5"><CardTitle>Parametric Builder: Recessed Lighting</CardTitle><CardDescription>Configure circuits once. Breakers, home runs and pricing follow automatically.</CardDescription></CardHeader>
          <CardContent className="space-y-4 pt-6"><RecessedBuilderFields inputs={inputs} onChange={setInputs} pricing={previewIsCurrent?pricing:undefined}/></CardContent>
        </Card>
      </div>
      <div className="min-w-0"><Card className="border-primary bg-secondary text-secondary-foreground xl:sticky xl:top-6">
        ...
        <CardContent className="space-y-5">
          ...
          <details className="rounded-md border p-3"><summary className="cursor-pointer text-sm">Advanced price overrides</summary><div className="mt-3 space-y-3">
            <div><Label htmlFor="recessed-labor-override">Internal Labor Cost Override ($)</Label><Input id="recessed-labor-override" type="number" min="0" step=".01" value={laborOverride} onChange={e=>setLaborOverride(e.target.value)} placeholder="Company-calculated"/></div>
            <div><Label htmlFor="recessed-price-override">Selling Price Override ($)</Label><Input id="recessed-price-override" type="number" min="0" step=".01" value={sellingPriceOverride} onChange={e=>setSellingPriceOverride(e.target.value)} placeholder="Company-calculated"/></div>
          </div></details>
          ...
          <Button className="w-full whitespace-normal" type="submit" value="quote" disabled={!canSave || blocked}>Generate Recessed Lighting Quote</Button>
