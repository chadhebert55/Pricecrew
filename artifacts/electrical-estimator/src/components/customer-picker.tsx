...
export function CustomerPicker({ customerId, customerName, customerEmail, onCustomerIdChange, onCustomerNameChange, onCustomerEmailChange, idPrefix }: CustomerPickerProps) {
  ...
  const { data: customers = [], isLoading } = useListCustomers(search.trim() ? { search: search.trim() } : undefined)
  ...
  return <div className="space-y-2 md:col-span-2">
    <div className="space-y-2">
      ...
      {search && <div className="max-h-36 overflow-y-auto rounded-md border bg-popover p-1">
        {isLoading ? <p className="p-2 text-sm text-muted-foreground">Searching customers…</p> : customers.length === 0 ? <p className="p-2 text-sm text-muted-foreground">No matching customers. Enter a new customer in the fields below.</p> : customers.map((customer) =>
